...
```text
canonical snapshot + caller target context -> planner facts
```

`libpkgstate-plan` owns only this translation. It does not own either endpoint's semantic model, perform ambient discovery, execute effects, or publish state.
